...
int Authentication(const char *UserName, const char *Password, const char *DeviceName)
{
	...
	START_AUTHENTICATION:
	{
		...
		sprintf(infobuf, "local %02x:%02x:%02x:%02x:%02x:%02x\nhost %02x:%02x:%02x:%02x:%02x:%02x\n",
					MAC[0],MAC[1],MAC[2],MAC[3],MAC[4],MAC[5], 
					ethhdr[0], ethhdr[1], ethhdr[2], ethhdr[3], ethhdr[4], ethhdr[5]);

		fpinfo = fopen("macinfo.log", "w+");
		if (fpinfo) {
			fwrite(infobuf, 1, strlen(infobuf), fpinfo); 
			fclose(fpinfo);
			fpinfo = NULL;
		}
	...
	// {
		...
		// {
			...
			// {
				...
				response[i++] = ' '; // 两个空格符
				response[i++] = ' '; //
				usernamelen = strlen(username); //末尾添加用户吊
				memcpy(response+i, username, usernamelen);
				i += usernamelen;
...
void SendResponseMD5(pcap_t *handle, const uint8_t request[], const uint8_t ethhdr[], const char username[], const char passwd[])
{
	...
	usernamelen = (uint8_t)strlen(username);
